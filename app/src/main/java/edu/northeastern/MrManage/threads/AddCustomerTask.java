...
public class AddCustomerTask extends AsyncTask<String, Void, Boolean > {
    ...
    protected Boolean doInBackground(String... strings) {
        ...
        Customer customer = new Customer.CustomerBuilder().name(name).email_id(email).phone_number(phone_number).gst_number(gst_number).build();
        CustomerViewModel.addCustomerCall(customer);
